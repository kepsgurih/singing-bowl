<script lang="ts">
  ...
  <div class="p-4 space-y-4 relative">
	...
	<div class="flex justify-between items-center">
	  <button on:click={prevMonth} class="px-3 py-1 bg-gray-200 rounded-md hover:bg-gray-300">&lt;</button>
	  <h2 class="text-lg font-semibold">{currentDate.format('MMMM YYYY')}</h2>
	  <button on:click={nextMonth} class="px-3 py-1 bg-gray-200 rounded-md hover:bg-gray-300">&gt;</button>
	</div>
	...
	<div class="grid grid-cols-7 gap-2 relative">
	  ...
	  {#each getDaysInMonth() as day}
		<button
		  ...
		  {isDateDisabled(day) ? 'bg-gray-100 text-gray-400 cursor-not-allowed' : 'bg-white hover:bg-purple-100'}"
		>
		  {day.date()}
	...
	{#if selectedDate}
	  <div class="mt-4 p-4 border rounded-lg bg-purple-50 space-y-2">
		<div class="flex justify-between items-center">
		  ...
		  <button on:click={() => selectedDate = null} class="text-sm text-purple-600 hover:underline">
			Tutup
		  </button>
		</div>
		...
		{#each getKelasForDate(selectedDate) as k}
		  <div class="text-sm text-gray-700">
			<button on:click={()=> alert('test pilih')} class="bg-gradient-to-r from-violet-200 to-pink-200 text-gray-800 px-5 font-bold py-2 rounded">⏰ {k.time}</button>
		  </div>
		{/each}
	  </div>
	{/if}
