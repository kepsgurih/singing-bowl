	...
	const applyFilters = () => {
		filteredBookings.set(
			data.book.filter((booking: any) => {
				return (
					(!filters.class ||
						booking.calendar.schedule.label.toLowerCase().includes(filters.class.toLowerCase())) &&
					(!filters.date ||
						new Date(booking.calendar.date).toISOString().split('T')[0] === filters.date) &&
					(!filters.isGroup ||
						(filters.isGroup === 'yes'
							? booking.calendar.schedule.isGroup
							: !booking.calendar.schedule.isGroup)) &&
					(!filters.location ||
						booking.calendar.kelas.toLowerCase().includes(filters.location.toLowerCase())) &&
					(!filters.fullName ||
						booking.user.fullName.toLowerCase().includes(filters.fullName.toLowerCase()))
				);
			})
		);
	};
						...
						{#each $filteredBookings as booking}
							<tr class="hover:bg-gray-50 transition-colors">
								<td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900"
									>{booking.calendar.schedule.label}</td
								>
								<td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900"
									>{booking.calendar.schedule.isGroup ? 'Yes' : 'No'}</td
								>
								...
								<td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500"
									>{booking.calendar.kelas}</td
								>
								<td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900"
									>{booking.user.fullName}</td
								>
								<td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500"
									>{booking.user.phone || 'N/A'}</td
								>
					...
					<div class="p-4 space-y-3">
						<div class="flex justify-between">
							...
							<span class="font-medium"
								>{booking.calendar.schedule.price
									? rupiahFormat(booking.calendar.schedule.price)
									: 'Free'}</span
							>
						</div>
