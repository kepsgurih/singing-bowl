...
<div class="p-8">
	<div class="flex justify-between items-center mb-6">
		...
		<button
			...
			on:click={() => goto('/dashboard/tree/new')}
		>
			+ New Link
		</button>
	</div>
		...
		{#each data.links as link}
			<div class="items-center p-4 bg-white rounded-lg shadow-sm">
				...
				<div class="flex gap-2 mt-4">
					<button
						on:click={() => goto(`/dashboard/tree/edit/${link.id}`)}
						class="bg-orange-400 px-3 py-1 rounded text-white hover:bg-yellow-500"
					>
						Edit
					</button>
					<form method="POST" class="inline">
						...
						<button
							type="submit"
							class="bg-rose-600 px-3 py-1 rounded text-white hover:bg-rose-500"
						>
							Delete
						</button>
					</form>
